...
This folder is the **design handoff** for the Threadwick Studio redesign: a full rework of the Studio shell that unifies a maker's library and the marketplace into one coherent, local-first product, designed for **both artists (design) and makers (make)** in a single app.
...
- **Build:** not started here — this hand-off intentionally contains the *design*, not the implementation. The existing `src/concept/` prototype in the app repo explored an earlier direction (“Two Front Doors”) and is **superseded** by this spec.
- **Deferred:** Creator Insights interior (a later phase).
